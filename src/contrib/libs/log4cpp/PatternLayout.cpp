...
#include <contrib/libs/log4cpp/PatternLayout.hh>
#include <contrib/libs/log4cpp/Priority.hh>
#include <contrib/libs/log4cpp/NDC.hh>
#include <contrib/libs/log4cpp/TimeStamp.hh>
#include <contrib/libs/log4cpp/FactoryParams.hh>
#include <memory>
...
namespace log4cpp {
    //---avikulin---
    //---Machine name (a.k.a. Node name) component (%N)
    struct NodeNameComponent : public PatternLayout::PatternComponent {
    ...
    //---avikulin---
    //---User name component (%U)
    struct UserNameComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            out << GetUsername().c_str();
        }
    };

    //---avikulin---
    //---Program version (%V)
    struct ProgramVersionComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            out << TP_VERSION;
        }
    };
    ...
    //---avikulin---
    //---Program name (%P)
    struct ProgramNameComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            out << GetProgramName().c_str(); 
        }
    };

    //---avikulin---
    //---Program directory (%D)
    struct ExecDirectoryComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            out << GetProgramDir().c_str(); 
        }
    };

        //---avikulin---
    //---Stack trace (%S)
    struct StackTraceComponent : public PatternLayout::PatternComponent {
        virtual void append(std::ostringstream& out, const LoggingEvent& event) {
            TBackTrace bt;
            bt.Capture();
            out << bt.PrintToString().c_str();
        }
    };
    ...
    void PatternLayout::setConversionPattern(const std::string& conversionPattern) {
        ...
        while (conversionStream.get(ch)) {
            if (ch == '%') {
                ...
                //--avikulin--
                //--add nodename pattern "%N"
                case 'N':
                    component = new NodeNameComponent();
                    break;
                
                //--avikulin--
                //--add username pattern "%U"
                case 'U':
                    component = new UserNameComponent();
                    break;

                //--avikulin--
                //--program version pattern "%V"
                case 'V':
                    component = new ProgramVersionComponent;
                    break;
                
                //--avikulin--
                //--add program name pattern "%P"
                case 'P':
                    component = new ProgramNameComponent();
                    break;
                
                //--avikulin--
                //--add exec directory path pattern "%D"
                case 'D':
                    component = new ExecDirectoryComponent();
                    break;
                
                //--avikulin--
                //--add stack trace pattern "%S"
                case 'S':
                    component = new StackTraceComponent();
                    break;
